...
public class RevController implements Initializable, ClientCallback {
	...
	private final ObservableList allergy_list = FXCollections
			.observableArrayList();
	private final ObservableList<Person> search_patient_results = FXCollections
			.observableArrayList();
	...
	private void runValidSearch() {
		...
		search_First_Name.setOnMouseExited(e -> {
			...
			try {
				matchingPeople = client.getServer().searchPersonByDetails(
						search_NHS_No.getText(), search_First_Name.getText(),
						search_Surname.getText(), search_DOB.getText(),
						search_Postcode.getText(),
	...
	private void buttonFunction() {
		...
		login.setOnAction(e -> {
			...
			bt1.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent event) {
					...
					String staff_LastName = null;
					String staff_FirstName = null;

					try {
						matchingPeople1 = client.getServer()
								.searchStaffByDetails(_user, db_pass);
		...
		UPGRADE.setOnAction(e -> {
			...
			if (potential != null) {
				...
				outputTextArea
						.appendText(potential + " is now an emergency!\n");
			}
		});
		...
		search_database
				.setOnAction(e -> {
					...
					outputTextArea.appendText(search_Surname.getText() + ", "
							+ search_First_Name.getText()
							+ " ready for Triage!\n");
					List<Person> matchingPeople = null;

					try {
						matchingPeople = client.getServer()
								.searchPersonByDetails(search_NHS_No.getText(),
										search_First_Name.getText(),
										search_Surname.getText(),
										search_DOB.getText(),
										search_Postcode.getText(),
										search_Telephone_No.getText());
					} catch (RemoteException ex) {
					...
					// If there were people matching the criteria, display them
					// to the user
					if (matchingPeople.size() > 0) {
						displayedPerson = matchingPeople.get(0);
						textfield_First_Name.setText(displayedPerson
								.getFirstName());
						...
						textfield_Blood_Group.setText(displayedPerson
								.getBloodGroup());
						textfield_Postcode.setText(displayedPerson
								.getPostcode());
						textfield_Telephone.setText(displayedPerson
								.getTelephone());

						// If the returned allergy is "null" set the allergy
						// box to display "None".
						if (displayedPerson.getAllergies().equalsIgnoreCase(
								"null")) {
							allergy.setValue(allergic[0]);
						} else {
							// Else set the allergy box value to the returned
							// alergy
		...
		emergency.setOnAction(e -> {
			...
			Patient emergency_patient = new Patient(new Person(
					textfield_NHS_Num.getText(), textfield_Title.getText(),
					textfield_First_Name.getText(),
					textfield_Surname.getText(), textfield_DOB.getText(),
					textfield_Address.getText(), textfield_Postcode.getText(),
					textfield_Telephone.getText(), textfield_Blood_Group
			...
			try {
				client.getServer().addPrimaryPatient(displayedPerson,
						tb1.isSelected(), tb2.isSelected(), tb3.isSelected(),
						tb4.isSelected(), tb5.isSelected(), tb6.isSelected());
			} catch (Exception e1) {
				e1.printStackTrace();
			}
		...
		urg.setOnAction(e -> {
			...
			Patient urgent_patient = new Patient(new Person(textfield_NHS_Num
					.getText(), textfield_Title.getText(), textfield_First_Name
					.getText(), textfield_Surname.getText(), textfield_DOB
					.getText(), textfield_Address.getText(), textfield_Postcode
					.getText(), textfield_Telephone.getText(),
		...
		semi_urg.setOnAction(e -> {
			...
			Patient semi_urgent_patient = new Patient(new Person(
					textfield_NHS_Num.getText(), textfield_Title.getText(),
					textfield_First_Name.getText(),
					textfield_Surname.getText(), textfield_DOB.getText(),
					textfield_Address.getText(), textfield_Postcode.getText(),
		...
		non_urg.setOnAction(e -> {
			...
			Patient non_urgent_patient = new Patient(new Person(
					textfield_NHS_Num.getText(), textfield_Title.getText(),
					textfield_First_Name.getText(),
					textfield_Surname.getText(), textfield_DOB.getText(),
					textfield_Address.getText(), textfield_Postcode.getText(),
	...
	private void treatmentRoomsView(ObservableList<Patient> o) {
		...
		Patient pat = new Patient(new Person("", null, "Ciaran", "Molloy",
	...
	private void toolTime() {
		...
		UPGRADE.setTooltip(new Tooltip(
